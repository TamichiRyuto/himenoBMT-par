...
#define MR(mt, n, r, c, d)                                                     \
  mt->m[(n) * mt->mrows * mt->mcols * mt->mdeps +                              \
        (r) * mt->mcols * mt->mdeps + (c) * mt->mdeps + (d)]
...
int main(int argc, char *argv[]) {
  ...
  if (cpu != 0.0)
    xgflops2 = (double)flop / cpu * 1.0e-9 * nn;

  printf("cpu : %f sec.\n", cpu);
  printf("Loop executed for %d times\n", nn);
  printf("Gosa : %e \n", gosa);
  printf("GFLOPS measured : %f\n", xgflops2);
  score = xgflops2 / 82.84 * 1000.0;
...
float jacobi(int* nn, int nnmax, Matrix *a, Matrix *b, Matrix *c, Matrix *p, Matrix *bnd, Matrix *wrk1, Matrix *wrk2) {
  ...
  for (n = 1; n <= nnmax; n++) {
    gosa = 0.0f;
    #pragma omp target teams loop reduction(+:gosa) collapse(3) 
    for (i = 1; i < imax; i++)
      for (j = 1; j < jmax; j++)
        for (k = 1; k < kmax; k++) {

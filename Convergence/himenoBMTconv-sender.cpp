...
float jacobi(int* nn, int nnmax, Matrix* _a, Matrix* _b, Matrix* _c, Matrix* _p, Matrix* _bnd, 
              Matrix* _wrk1, Matrix* _wrk2) {
  ...
  /* Computing Task */
  auto compute_task=stdexec::schedule(gpu_sched)
    | stdexec::bulk(gridSize, [=](int idx) {
      if (!isConverged) {
  ...
  /* Convergence Check Task */
  float sum_gosa;
  auto judge_task = stdexec::schedule(cpu_sched) | stdexec::then([&]() {
  ...
  /* Submit Task to Schedulers */
  for (n = 1; n < nnmax; n++) {
    stdexec::start_detached(compute_task);
    stdexec::start_detached(judge_task);
    if (stopToken.stop_requested()) break;
  } /* end n loop */

  stdexec::sync_wait(compute_task);
  stdexec::sync_wait(judge_task);
  *nn = n;
  delete[] gosa;
  return sum_gosa;
}

...
float jacobi_for_each_n(int n, Matrix* M1, Matrix* M2, Matrix* M3, Matrix* M4, Matrix* M5, Matrix* M6, Matrix* M7);
...
int main(int argc, char* argv[]) {
  ...
  // gosa = jacobi_for_each_n(nn, &a, &b, &c, &p, &bnd, &wrk1, &wrk2);
  cpu1 = second();
  cpu = cpu1 - cpu0;
...
*/
float jacobi(int nn, Matrix* _a, Matrix* _b, Matrix* _c, Matrix* _p, Matrix* _bnd, 
...
*/
float jacobi_for_each_n(int nn, Matrix* _a, Matrix* _b, Matrix* _c, Matrix* _p, Matrix* _bnd, 
                        Matrix* _wrk1, Matrix* _wrk2) {
  /* -stdpar=gpu to deal with structure pointer members that cannot be changed */
  Matrix a = *_a;
  Matrix b = *_b;
  ...
  float sum_gosa = std::reduce(std::execution::par_unseq, gosa, gosa + gridSize, 0.0f); 
